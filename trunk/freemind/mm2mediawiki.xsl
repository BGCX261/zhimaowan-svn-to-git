...
<!--
    (c) by Sun Liwen<slw4qd@gmail.com>, 2008
    This file is licensed under the LGPL.
    
    $Revision: 7 $:
    $Author: slw4qd $:
    $Date: 2009-01-05 09:39:36 +0600 (Mon, 05 Jan 2009) $:
-->
            ...
            <xsl:apply-templates select=".." mode="childoutput">
              <xsl:with-param name="nodeText">
                ...
                <xsl:call-template name="spaces">
                  <xsl:with-param name="count" select="($depth - 2) * 1"/>
                </xsl:call-template>
                <!-- Do we have text with a LINK attribute? -->
                <xsl:choose>
                  <xsl:when test="((position() = 1) and (count(following-sibling::*) = 0)) and ($depth &lt; 3)">
                    <xsl:text></xsl:text>
                    <xsl:value-of select="@TEXT"/>
                  </xsl:when>
